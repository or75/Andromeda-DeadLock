...
auto CAndromedaMenu::InitColors() -> void
{
	...
	m_ColorsMisc[XorStr( "Misc" )] =
	{
		{ &Settings::Colors::Misc::SkyColor[0] , XorStr( "Sky Color" ) },
	};
...
auto CAndromedaMenu::OnRenderMisc() -> void
{
	if ( m_bMiscWindow )
	{
		...
		{
			ImGui::BeginGroup();
			ImGui::BeginChild( XorStr( "##AndromedaMiscLeftChild" ) , ImVec2( ImGui::GetContentRegionAvail().x / 2.5f , ImGui::GetContentRegionAvail().y ) , ImGuiChildFlags_Borders );

			RenderCheckBox( XorStr( "Sky Color Changer" ) , XorStr( "##Settings.Misc.SkyColorChanger" ) , Settings::Misc::SkyColorChanger );
			RenderCheckBox( XorStr( "Night Mode" ) , XorStr( "##Settings.Misc.NightMode" ) , Settings::Misc::NightMode );
